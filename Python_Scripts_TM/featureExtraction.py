...
def main(data,classifications):
    ...
    features = np.zeros((len(data),384))
    ...
    for i in range(0,len(data)):
        ...
        xsnr = stats.signaltonoise(xData)
        ysnr = stats.signaltonoise(yData)
        zsnr = stats.signaltonoise(zData)
       ...
       # xWaveletFeats = waveletFeatures(xData)
       # yWaveletFeats = waveletFeatures(yData)
        #zWaveletFeats = waveletFeatures(zData)
        #features[i][144:192] = np.concatenate([xWaveletFeats, yWaveletFeats, zWaveletFeats])
